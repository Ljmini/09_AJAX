...
public class AddService implements MemberService {
	...
	public void execute(HttpServletRequest request, HttpServletResponse response) throws IOException{
		...
		// id, name, gender, address 파라미터로 처리하기
		...
		// Mybatis의 경우 PersistanceException 발생 -> try-catch로 잡아보자!
		...
		PrintWriter out = null;
 		...
 		try {
 			...
 			// 성공했을 때 응답은 jQuery.ajax의 success 프로퍼티로 넘어간다!
 			...
 			// 실패했을 때(응답이 오지 x) 응답으로 status!=200인 상황! (기본적으로 서버 측 오류이면 status==500인 상황)
 			
 			// 실패 했을 때 응답은 alert 할 수 있도록 text 반환 처리 (내가 정한거~~~)
